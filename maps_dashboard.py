...
# =============================
# Helpers (with HTML minification + lazy render)
# =============================
...
def lazy_map(label: str, path: Path, height: int, width_px: Optional[int] = None, minify: bool = True):
    """
    Lazy-render: show a button first; only inject the heavy HTML when clicked.
    Keeps initial DOM light and avoids building huge iframes up front.
    """
    container = st.container()
    if st.button(f"▶️ Load {label}", key=f"btn_{label}"):
        with st.spinner(f"Rendering {label}…"):
            with container:
                render_html_now(path, height=height, width_px=width_px, minify=minify)
    else:
        st.caption("Map not loaded yet. Click the button above to render it.")
...
# -----------------------------
# FULL-WIDTH (overlay map) — LAZY LOAD
# -----------------------------
st.subheader("Overlay Results")
lazy_map("Overlay Results Map", FULL_CENTER_PATH, height=full_h, width_px=2200, minify=True)
...
# -----------------------------
# BOTTOM: two maps — TABS + LAZY LOAD (only load when tab visible & clicked)
# -----------------------------
st.subheader("Diversity Heatmaps")
tab_left, tab_right = st.tabs(["Personal Diversity", "Shannon Index"])

with tab_left:
    st.markdown("##### Personal Diversity Heatmap")
    lazy_map("Personal Diversity Heatmap", BOTTOM_LEFT_MAP_PATH, height=pair_h, width_px=1200, minify=True)
...
with tab_right:
    ...
    lazy_map("Shannon Index Heatmap", BOTTOM_RIGHT_MAP_PATH, height=pair_h, width_px=1200, minify=True)
